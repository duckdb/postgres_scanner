...
WHERE nspname='%s' AND relname='%s'
  ...
  if (res->Count() != 1) {
    throw InvalidInputException("Postgres table \"%s\".\"%s\" not found",
...
static void ProcessValue(data_ptr_t value_ptr, idx_t value_len,
  ...
  case LogicalTypeId::TIME_TZ: {
    D_ASSERT(bind_data->columns[col_idx].attlen ==
  ...
  case LogicalTypeId::INTERVAL: {
    if (bind_data->columns[col_idx].atttypmod != -1) {
      throw IOException("Interval with unsupported typmod %d",
    ...
    res.months =
        ntohl(Load<uint32_t>(value_ptr + sizeof(uint64_t) + +sizeof(uint32_t)));
...
PostgresInitLocalState(ExecutionContext &context, TableFunctionInitInput &input,
  ...
  if (!PostgresParallelStateNext(context.client, input.bind_data, *local_state,
                                 gstate)) {
...
static void PostgresScan(ClientContext &context, TableFunctionInput &data,
  ...
  while (true) {
    if (local_state->done &&
        !PostgresParallelStateNext(context, data.bind_data, *local_state,
                                   *gstate)) {
      return;
    }
