...
void PostgresBinaryParser::ReadValue(const LogicalType &type, const PostgresType &postgres_type, Vector &out_vec,
	...
	case LogicalTypeId::DOUBLE: {
		if (postgres_type.info == PostgresTypeAnnotation::NUMERIC_AS_DOUBLE) {
			...
			FlatVector::GetData<double>(out_vec)[output_offset] = double_value;
			break;
		}
	...
	case LogicalTypeId::DECIMAL: {
		...
		case PhysicalType::INT16: {
			...
			if (!CheckDecimalKindSetNull(out_vec, output_offset, dec.kind)) {
				FlatVector::GetData<int16_t>(out_vec)[output_offset] = dec.value;
			}
			break;
		}
		case PhysicalType::INT32: {
			...
			if (!CheckDecimalKindSetNull(out_vec, output_offset, dec.kind)) {
				FlatVector::GetData<int32_t>(out_vec)[output_offset] = dec.value;
			}
			break;
		}
		case PhysicalType::INT64: {
			...
			if (!CheckDecimalKindSetNull(out_vec, output_offset, dec.kind)) {
				FlatVector::GetData<int64_t>(out_vec)[output_offset] = dec.value;
			}
			break;
		}
		case PhysicalType::INT128: {
			...
			if (!CheckDecimalKindSetNull(out_vec, output_offset, dec.kind)) {
				FlatVector::GetData<hugeint_t>(out_vec)[output_offset] = dec.value;
			}
			break;
		}
		default:
